...
def getAttrbs(ctx, css, obj):
	st_ss, st_nontdm, st_is, st_ps = [set() for i in range(4)]  # サービス名、TypeDescriptionオブジェクトを取得できないサービス名、インターフェイス名を入れる集合、プロパティ名を入れる集合を初期化。
	...
	if isinstance(obj, str): # objが文字列(IDL名)のとき
		...
		if j:
			...
			if typcls == SERVICE:  # サービスの時
				st_ss.add(j.Name)
				args = st_ss, st_is, st_ps, [j]
				getServiceChild(args)
			elif typcls == INTERFACE:  # インターフェイスの時
				st_is.add(j.Name)
				getInterfaceChild(st_is, [j])	
	else:  # objが文字列以外の時
		if hasattr(obj, "getSupportedServiceNames"):  # オブジェクトがサービスを持っているとき。
			...
			if st_ss:
				args = st_ss, st_is, st_ps, [_idl_check(i) for i in st_ss]
				getServiceChild(args)  # TypeDescriptionオブジェクトに変換して渡す。	
		if hasattr(obj, "getTypes"):  # サービスを介さないインターフェイスがある場合。elifにしてはいけない。
			...
			if types:  # type型が取得できたとき。
				...
				getInterfaceChild(st_is, types)
	return st_ss, st_nontdm, st_is, st_ps
def getServiceChild(args):
	st_ss, st_is, st_ps, tdms = args
	for j in tdms:
		lst_std = list(j.getMandatoryServices())
		lst_std.extend(j.getOptionalServices())
		if lst_std:
			st_ss.update(i.Name for i in lst_std)
			args = st_ss, st_is, st_ps, lst_std
			getServiceChild(args)  # TypeDescriptionオブジェクトに変換して渡す。
		lst_itd = list(j.getInterface())  # new-styleサービスのインターフェイスを取得。TypeDescriptionオブジェクト。
...
def createTree(args, obj):
	...
	st_ss, st_nontdm, st_is, st_ps = args
